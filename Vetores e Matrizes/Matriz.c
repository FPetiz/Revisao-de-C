 ...
 * a soma dos elementos que estao na diagonal secundaria. E calcule e imprima a sua transposta.
 * -> Mistura dos exercícios 8, 9, 10, 11 e 12 da lista 4 de programacaodescomplicada.wordpress.com. */

#include <stdio.h>

int soma_acimaD(int m[3][3]);
...
int main(){
    ...
    soma_diagP = soma_diagonalP(matriz);
    soma_diagS = soma_diagonalS(matriz);
    ...
    printf("\nMatriz transposta:\n");
    for(j = 0; j < 3; j++){         //  Para a transposta trato o j da originl como linha
        for(i = 0; i < 3; i++){     //  e o i da oriinal como coluna
            matriz_transposta[j][i] = matriz[i][j];  
            printf("%3d", matriz_transposta[j][i]); // %3d para deixar um espaço horizontal entre os múmeros maior 
        }printf("\n");                              // que um espaço e menor que um tab: \t
    }
    
    return 0;
}
int soma_acimaD(int m[3][3]){  // Os indices das colunas são maiores que os das linhas
    int i, j, soma = 0;        // nos elementos acima da diagonal principal: 00 01 02
                               //                                            10 11 12
